    ...
    {editMode.introduction ? (
      <>
        <label className="text-sm font-semibold text-gray-600">
          LinkedIn URL (for Active Agent Assistant)
        </label>
        <input
          type="text"
        ...
        <button
          ...
          onClick={async () => {
            if (!formData.linkedin_url) return alert("Enter a LinkedIn URL first.");

            try {
              const res = await fetch(
                "https://tyxhurtlifaufindxhex.supabase.co/functions/v1/linkedin-summary",
                {
                  method: "POST",
                  headers: {
              ...
              if (json.summary) {
                setFormData((prev) => ({
                  ...prev,
                  linkedin_summary: json.summary,
                }));
              } else {
                alert("No summary found or could not extract one.");
              }
            } catch (error) {
        ...
        >
          ✨ Use Active Agent Assistant to generate intro
        </button>
